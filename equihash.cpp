...
template<size_t WIDTH>
bool StepRow<WIDTH>::IsZero(size_t len) const
{
                               ...
                               const std::function<bool(EhSolverCancelCheck)> cancelled)
{
    ...
    for (int r = 1; r < K && X.size() > 0; r++) {
        ...
        std::cout << "- Finding collisions\n";
        int groupCount = 0;
        std::map<int, int> groupSizes;
        int pairsCount = 0;
        std::map<int, int> pairsFromGroups;
        int i = 0;
        int posFree = 0;
        ...
        while (i < X.size() - 1) {
            ...
            for (int l = 0; l < j - 1; l++) {
                for (int m = l + 1; m < j; m++) {
                    ...
                    if (DistinctIndices_2(X[i+l], X[i+m], hashLen, lenIndices)) {
                                   ...
                                   const std::function<bool(EhSolverCancelCheck)> cancelled)
{
    ...
    std::cout << "Found " << partialSolns.size() << " partial solutions\n";
    
    // Now for each solution run the algorithm again to recreate the indices
    std::cout << "Culling solutions\n";
    ...
    std::cout << "- Number of invalid solutions found: " << invalidCount << "\n";
...
bool Equihash<N,K>::IsValidSolution(const eh_HashState& base_state, std::vector<unsigned char> soln)
{
    if (soln.size() != SolutionWidth) {
        std::cout << "Invalid solution length: " << soln.size() << " (expected " << SolutionWidth << ")\n";
        return false;
    }
    ...
    while (X.size() > 1) {
        ...
        for (int i = 0; i < X.size(); i += 2) {
            if (!HasCollision(X[i], X[i+1], CollisionByteLength)) {
                std::cout << "Invalid solution: invalid collision length between StepRows\n";
                std::cout << "X[i]   = " << X[i].GetHex(hashLen) << "\n";
                std::cout << "X[i+1] = " << X[i+1].GetHex(hashLen) << "\n";
                return false;
            }
            if (X[i+1].IndicesBefore(X[i], hashLen, lenIndices)) {
                return false;
                std::cout << "Invalid solution: Index tree incorrectly ordered\n";
            }
            if (!DistinctIndices(X[i], X[i+1], hashLen, lenIndices)) {
                std::cout << "Invalid solution: duplicate indices\n";
                return false;
            }
...
template std::vector<unsigned char> FullStepRow<2054ul>::GetIndices(size_t, size_t, size_t) const;

template FullStepRow<1030ul>::FullStepRow(const unsigned char*, size_t, size_t, size_t, eh_index);

template FullStepRow<1030ul>::FullStepRow(const FullStepRow<1030ul>&, const FullStepRow<1030ul>&, size_t, size_t, int);

template FullStepRow<2054ul>::FullStepRow(const FullStepRow<1030ul>&, const FullStepRow<1030ul>&, size_t, size_t, int);

template bool HasCollision<1030ul>(StepRow<1030ul>&, StepRow<1030ul>&, int);
